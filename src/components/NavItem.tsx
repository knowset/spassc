...
const NavItemContent: FC<NavItemContentProps> = ({ children, textSize, p, border, bg }) => {
    return (
        <div  className={`t-select-none t-cursor-pointer t-flex t-flex-row t-justify-center t-items-center t-text-center ${ textSize ? textSize : "t-text-sm 2xs:t-text-base xs:t-text-lg lg:t-text-xl"} ${ p ? p : "t-py-2 t-px-2 lg:t-px-4"} t-text-black sm:hover:t-bg-indigo-200 ${border ? border : ""} t-transition-colors t-duration-200`}>
            { children }
        </div>
    );
}
